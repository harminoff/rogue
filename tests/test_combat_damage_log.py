...
class CombatDamageLogTests(unittest.TestCase):
    def test_combat_damage_hook_is_wired_to_gui_log(self):
        ...
        self.assertIn("void rogue_frontend_record_damage(int dealt, int taken);", frontend_h)
        self.assertIn("void rogue_allegro_record_damage(int dealt, int taken);", frontend_c)
        self.assertIn("rogue_frontend_record_damage(damage_done, 0);", fight_c)
