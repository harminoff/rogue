...
int type;
{
    ...
    if (rogue_frontend_is_tiles() && !slow_invent)
	...
	switch (n_objs++)
	{
		...
		if (rogue_frontend_is_tiles() && !slow_invent)
		    rogue_frontend_text_overlay_add(inv_temp);
		break;
	    /*
...
int type;
{
	...
	return TRUE;
    if (type == CALLABLE && (obj->o_type == SCROLL || obj->o_type == POTION
	|| obj->o_type == RING || obj->o_type == STICK))
	return TRUE;
    return FALSE;
}
...
static char
tile_pick_pack_letter(purpose, type)
char *purpose;
int type;
{
    register struct linked_list *item;
    ...
    matches = 0;
    rogue_frontend_text_overlay_begin("Inventory");
    for (item = pack, ch = 'a'; item != NULL; item = next(item), ch++)
    {
	if (!pack_type_matches(item, type))
	    continue;
	obj = (struct object *) ldata(item);
	sprintf(tile_line, "%c) %s", ch, inv_name(obj, FALSE));
	...
	purpose = "choose";
    sprintf(prompt, "Letters select, arrows move, Enter to %s, Esc cancels",
	purpose);
...
int type;
{
	...
	for (;;)
	{
	     ...
	     */
	    if (ch == ESCAPE || ch == CTRL('G') || ch == '\0' || ch == ' '
		|| ch == '\n' || ch == '\r')
	    {
		after = FALSE;
		msg("");
