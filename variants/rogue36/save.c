...
char *message;
{
	...
	rogue_frontend_notice("Restore Failed", message);
    else
	printf("%s\n", message);
    return FALSE;
}
...
char **envp;
{
    ...
    {
	sprintf(buf, "%s: %s", file, strerror(errno));
...
/*    lseek(inf, 0L, 0);*/
	...
	{
	    return restore_error("Sorry, saved game is not in the same file.");
	}
	else if (sbuf2.st_ctime - sbuf.st_ctime > 15)
	{
	    return restore_error("Sorry, file has been touched.");
	}
    ...
    {
	endwin();
	sprintf(buf, "Saved game needs %d screen lines; current screen has %d.",
	    slines, LINES);
	return restore_error(buf);
    }
    ...
    {
	endwin();
	sprintf(buf, "Saved game needs %d screen columns; current screen has %d.",
	    scols, COLS);
	return restore_error(buf);
    }
    ...
    {
	endwin();
	return restore_error("Cannot restore file.");
    }
