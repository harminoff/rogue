...
static tile_option(), tile_option_line(), tile_change_option();
...
static
tile_option_line(key, op, line, line_size)
char key;
OPTION *op;
char *line;
int line_size;
{
	...
	sprintf(value, "%s", *(bool *) op->o_opt ? "True" : "False");
    else
	sprintf(value, "%s", (char *) op->o_opt);

    sprintf(line, "%c) %s(\"%s\"): %s", key, op->o_prompt,
	op->o_name, value);
}

static
tile_change_option(op)
OPTION *op;
{
    char updated[80];
	...
	*(bool *) op->o_opt = (bool) !*(bool *) op->o_opt;
    else if (rogue_frontend_text_input("Option", op->o_prompt,
	(char *) op->o_opt, updated, sizeof(updated)))
	strucpy((char *) op->o_opt, updated, strlen(updated));
}
...
WINDOW *win;
{
    ...
    if (rogue_frontend_is_tiles())
    {
	if (rogue_frontend_text_input("Input", "Value:",
	    opt, buf, sizeof(buf)))
	{
