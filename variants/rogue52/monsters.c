 ...
 */
static char
tile_pick_genocide_monster()
{
    register int i;
    char line[MAXSTR];
    char choice;

    rogue_frontend_text_overlay_begin("Genocide");
    for (i = 0; i < 26; i++)
    {
	sprintf(line, "%c) %s", (char)('A' + i), monsters[i].m_name);
	rogue_frontend_text_overlay_add(line);
    }
    choice = rogue_frontend_text_overlay_pick(
	"Letters select, arrows move, Enter selects, Esc cancels");
