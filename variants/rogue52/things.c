...
static char *line_overlay_title = "Inventory";

static char
tile_pick_discovery_type()
{
    char answer;

    rogue_frontend_text_overlay_begin("Discoveries");
    rogue_frontend_text_overlay_add("!) Potions");
    rogue_frontend_text_overlay_add("?) Scrolls");
    rogue_frontend_text_overlay_add("=) Rings");
    rogue_frontend_text_overlay_add("/) Sticks");
    rogue_frontend_text_overlay_add("*) All discovered items");
    answer = rogue_frontend_text_overlay_pick(
	"Letters/select a type, arrows move, Enter selects, Esc cancels");
...
discovered()
{
    ...
    if (ch == '*')
    {
	line_overlay_title = "Discoveries";
	print_disc(POTION);
	add_line("");
	print_disc(SCROLL);
	add_line("");
...
char *fmt, *arg;
{
    ...
    if (line_cnt == 0)
    {
		...
		rogue_frontend_text_overlay_begin(line_overlay_title);
    }
    if (slow_invent)
    {
	if (*fmt != '\0')
	...
	if (line_cnt >= LINES - 1 || fmt == NULL)
	{
	    ...
	    newpage = TRUE;
	    line_cnt = 0;
	    if (fmt != NULL && rogue_frontend_is_tiles())
		rogue_frontend_text_overlay_begin(line_overlay_title);
	}
	if (fmt != NULL && !(line_cnt == 0 && *fmt == '\0'))
	{
	    ...
	    {
		if (strchr(fmt, '%') != NULL)
		    sprintf(tile_line, fmt, arg);
		else
		    sprintf(tile_line, "%s", fmt);
