...
tryagain:
	...
	while (i) {
		if (rogue_frontend_is_tiles()) {
			...
			for (ii = 0; ii < MAXMONS; ii++) {
				if (monsters[ii].m_lev.l_lev < 0)
					continue;
				sprintf(line, "%c) %s", monsters[ii].m_show,
					monsters[ii].m_name);
				rogue_frontend_text_overlay_add(line);
			}
			c = rogue_frontend_text_overlay_pick(
				"Letters select, arrows move, Enter chooses, Esc cancels");
