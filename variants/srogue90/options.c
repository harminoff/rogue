	...
	if (rogue_frontend_is_tiles()) {
		for (;;) {
			...
			for (op = optlist; op < &optlist[NUM_OPTS]; op++) {
				wh = op - optlist;
				sprintf(line, "%c) %s%s", 'a' + wh,
					op->o_prompt, op->o_opt);
				rogue_frontend_text_overlay_add(line);
			}
			picked = rogue_frontend_text_overlay_pick(
				"Letters select, arrows move, Enter edits, Esc closes");
			rogue_frontend_text_overlay_clear();
			if (picked == ESCAPE || picked == '\0' || picked == ' ')
				break;
			wh = picked - 'a';
			if (wh < 0 || wh >= NUM_OPTS)
				continue;
			op = &optlist[wh];
			strcpy(input, op->o_opt);
			if (rogue_frontend_text_input("Options", op->o_prompt,
			    op->o_opt, input, sizeof input))
				strucpy(op->o_opt, input, strlen(input));
		}
		after = FALSE;
		return;
	}
...
WINDOW *awin;
{
	...
	if (rogue_frontend_is_tiles()) {
		strcpy(buf, opt);
		if (!rogue_frontend_text_input("Text Input", "Enter text",
		    opt, buf, sizeof buf))
			return QUIT;
		if (buf[0] != '\0')
