	...
	when P_TFIND:
		...
		if (lvl_obj != NULL && !curse) {
			...
			if (rogue_frontend_is_tiles())
				rogue_frontend_text_overlay_begin("Magic Sensed");
			else
				wclear(hw);
			for (mobj = lvl_obj; mobj != NULL; mobj = next(mobj)) {
				...
				if (is_magic(tp)) {
					show = TRUE;
					if (rogue_frontend_is_tiles()) {
						sprintf(line, "Magic aura at row %d, col %d",
							tp->o_pos.y + 1, tp->o_pos.x + 1);
			...
			for(titem = mlist; titem != NULL; titem = next(titem)) {
				...
				for(pitem=th->t_pack;pitem!=NULL;pitem=next(pitem)) {
					if (is_magic(ldata(pitem))) {
						show = TRUE;
						if (rogue_frontend_is_tiles()) {
							sprintf(line, "Magic aura carried at row %d, col %d",
								th->t_pos.y + 1, th->t_pos.x + 1);
							rogue_frontend_text_overlay_add(line);
						}
						else
