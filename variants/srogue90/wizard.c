...
bool fscr;
{
	...
	if (fscr)
		...
		if (rogue_frontend_is_tiles()) {
			rogue_frontend_text_overlay_begin("Create Object");
			for (otype = 0; otype < NUMTHINGS; otype++) {
				if (otype != TYP_AMULET || wizard) {
					mf = &thnginfo[otype];
					sprintf(line, "%c) %s", mf->mf_show,
						things[otype].mi_name);
					rogue_frontend_text_overlay_add(line);
				}
			}
			for (otype = 0; otype < MAXMONS; otype++) {
				sprintf(line, "%c) monster: %s",
					monsters[otype].m_show,
					monsters[otype].m_name);
				rogue_frontend_text_overlay_add(line);
			}
			ch = rogue_frontend_text_overlay_pick(
				"Letters select, arrows move, Enter creates, Esc cancels");
	...
	if (nogood) {
		...
		if (rogue_frontend_is_tiles()) {
			rogue_frontend_text_overlay_begin(oname);
			for (ii = 0 ; ii < msz ; ii++) {
				sprintf(line, "%c) %s", ii + 'a', wmi->mi_name);
				rogue_frontend_text_overlay_add(line);
				wmi++;
			}
			sprintf(prbuf, "Letters select, arrows move, Enter chooses %s, Esc cancels",
				oname);
			ch = rogue_frontend_text_overlay_pick(prbuf);
			rogue_frontend_text_overlay_clear();
			if (ch == ESCAPE || ch == '\0') {
				msg("");
				return;
