 ...
 */
static void
format_help_line(strp, line, line_size)
struct h_list *strp;
char *line;
int line_size;
{
	...
	if (strp->h_ch)
		sprintf(key, "%s", unctrl(strp->h_ch));
	else
		key[0] = '\0';
...
{
	reg char ch, *str;
	reg struct h_list *hp;
	char line[LINLEN];
	static struct h_list ident_list[] = {
		{'|',		"the wall of a room"},
		{'-',		"the wall of a room"},
		{GOLD,		"gold"},
		{STAIRS,	"passage leading up/down"},
		{DOOR,		"door"},
		{FLOOR,		"room floor"},
		{PLAYER,	"you"},
		{PASSAGE,	"passage"},
		...
		{SLEEPTRAP,	"sleeping gas trap"},
		{BEARTRAP,	"bear trap"},
		{TELTRAP,	"teleport trap"},
		{DARTTRAP,	"dart trap"},
		{POOL,		"magic pool"},
		{POTION,	"potion"},
		{SCROLL,	"scroll"},
	...
	if (what == 0) {
		if (rogue_frontend_is_tiles()) {
			...
			for (hp = ident_list; hp->h_ch != '\0'; hp++) {
				sprintf(line, "%s) %s", unctrl(hp->h_ch),
					hp->h_desc);
				rogue_frontend_text_overlay_add(line);
			}
			for (ch = 'A'; ch <= 'Z'; ch++) {
				sprintf(line, "%c) %s", ch,
					monsters[midx(ch)].m_name);
				rogue_frontend_text_overlay_add(line);
			}
			for (ch = 'a'; ch <= 'z'; ch++) {
				sprintf(line, "%c) %s", ch,
...
shell()
{
	...
	if (rogue_frontend_is_tiles()) {
		rogue_frontend_notice("Shell Escape",
			"Shell escape is only available from terminal ASCII mode.");
		msg("Shell escape is only available from terminal ASCII mode.");
		return;
	}
	/*
