...
int type;
{
	...
	if (list == NULL) {			/* empty list */
		...
		rogue_frontend_text_overlay_begin(
			list == pack ? "Inventory" : "Items");
...
picky_inven()
{
	...
	if (pack == NULL)
		...
		rogue_frontend_text_overlay_begin("Inventory");
		for (ch='a',item=pack; item != NULL; item=next(item),ch=npch(ch)) {
			sprintf(tile_line, "%c) %s", ch, inv_name(OBJPTR(item), FALSE));
			rogue_frontend_text_overlay_add(tile_line);
		}
